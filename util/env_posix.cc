...
class PosixRandomAccessFile final : public RandomAccessFile
{
  public:
    ...
    Status
    Read(uint64_t offset, size_t n, Slice *result, char *scratch) const override
    {
        int fd = fd_;
        if (!has_permanent_fd_) {
...
class PosixWritableFile final : public WritableFile
{
  ...
  private:
    ...
    static std::string Dirname(const std::string &filename)
    {
        ...
        if (separator_pos == std::string::npos) { return std::string("."); }
    ...
    {
        std::string::size_type separator_pos = filename.rfind('/');
        if (separator_pos == std::string::npos) { return Slice(filename); }
        // The filename component should not contain a path separator. If it
        // does, the splitting was done incorrectly.
        assert(filename.find('/', separator_pos + 1) == std::string::npos);

        return Slice(
            filename.data() + separator_pos + 1,
  ...
  public:
        ...
        RandomAccessFile **result) override
    {
        *result = nullptr;
        ...
        if (status.ok()) {
            void *mmap_base = ::mmap(
                /*addr=*/nullptr, file_size, PROT_READ, MAP_SHARED, fd, 0);
        ...
        ::close(fd);
        if (!status.ok()) { mmap_limiter_.Release(); }
        return status;
    }

    Status
    NewWritableFile(const std::string &filename, WritableFile **result) override
    {
        int fd = ::open(
            filename.c_str(),
        ...
        std::vector<std::string> *result) override
    {
        ...
        if (dir == nullptr) { return PosixError(directory_path, errno); }
        struct ::dirent *entry;
        ...
        *lock = nullptr;
        ...
        if (!locks_.Insert(filename)) {
            ...
            return Status::IOError(
                "lock " + filename, "already held by process");
        }
    ...
    void StartThread(
        void (*thread_main)(void *thread_main_arg),
    ...
    void *background_work_arg)
{
    ...
    if (!started_background_thread_) {
        ...
        std::thread background_thread(
            PosixEnv::BackgroundThreadEntryPoint, this);
        background_thread.detach();
    }

    // If the queue is empty, the background thread may be waiting for work.
    if (background_work_queue_.empty()) { background_work_cv_.Signal(); }

    background_work_queue_.emplace(
...
#endif // !defined(NDEBUG)
        static_assert(
            sizeof(env_storage_) >= sizeof(EnvType),
