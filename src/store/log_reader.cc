    ...
    while (true) {
        ...
        case kFullType:
            if (in_fragmented_record) {
                ...
                if (!scratch->empty()) {
                    ReportCorruption(
                        scratch->size(), "partial record without end(1)");
                }
            }
        ...
        case kFirstType:
            if (in_fragmented_record) {
                ...
                if (!scratch->empty()) {
                    ReportCorruption(
                        scratch->size(), "partial record without end(2)");
                }
            }
            prospective_record_offset = physical_record_offset;
        ...
        case kMiddleType:
            if (!in_fragmented_record) {
                ReportCorruption(
                    fragment.size(), "missing start of fragmented record(1)");
            } else {
        ...
        case kLastType:
            if (!in_fragmented_record) {
                ReportCorruption(
                    fragment.size(), "missing start of fragmented record(2)");
            } else {
                scratch->append(fragment.data(), fragment.size());
        ...
        default: {
            char buf[40];
            std::snprintf(
                buf, sizeof(buf), "unknown record type %u", record_type);
            ReportCorruption(
                (fragment.size() +
