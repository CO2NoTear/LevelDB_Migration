...
// #include "store/log_reader.h"
// #include "store/log_writer.h"
...
#include "store/port.h"
#include "table/block.h"
#include "table/merger.h"
#include "table/two_level_iterator.h"
#include "store/coding.h"
#include "util/logging.h"
#include "store/mutexlock.h"
...
DBImpl::~DBImpl()
{
    ...
    if (db_lock_ != nullptr) { env_->UnlockFile(db_lock_); }
...
Status DBImpl::Recover(VersionEdit *edit, bool *save_manifest)
{
    ...
    if (!expected.empty()) {
        ...
        return Status::Corruption(
            buf, TableFileName(dbname_, *(expected.begin())));
    }
    ...
    SequenceNumber *max_sequence)
{
    ...
    log::Reader reader(
        file, &reporter, true /*checksum*/, 0 /*initial_offset*/);
    Log(options_.info_log,
        "Recovering log #%llu",
...
Status DBImpl::DoCompactionWork(CompactionState *compact)
{
    ...
    while (input->Valid() && !shutting_down_.load(std::memory_order_acquire)) {
        ...
        if (!ParseInternalKey(key, &ikey)) {
            ...
            if (!has_current_user_key ||
                ...
                current_user_key.assign(
                    ikey.user_key.data(), ikey.user_key.size());
    ...
    if (status.ok()) { status = InstallCompactionResults(compact); }
    if (!status.ok()) { RecordBackgroundError(status); }
    VersionSet::LevelSummaryStorage tmp;
...
bool DBImpl::GetProperty(const Slice &property, std::string *value)
{
    ...
    if (in.starts_with("num-files-at-level")) {
        ...
        *value = versions_->current()->DebugString();
        ...
        if (mem_) { total_usage += mem_->ApproximateMemoryUsage(); }
        if (imm_) { total_usage += imm_->ApproximateMemoryUsage(); }
        char buf[50];
        std::snprintf(
            buf,
            sizeof(buf),
    ...
    *dbptr = nullptr;
    ...
    if (s.ok() && impl->mem_ == nullptr) {
        ...
        s = options.env->NewWritableFile(
            LogFileName(dbname, new_log_number), &lfile);
        if (s.ok()) {
